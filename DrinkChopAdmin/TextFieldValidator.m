...
-(void)addDistinctValidationTo:(NSString *)txtConfirm withMsg:(NSString *)msg{
    NSDictionary *dic=[[NSDictionary alloc] initWithObjectsAndKeys:txtConfirm,@"distinct",msg,@"msg", nil];
    [arrRegx addObject:dic];
}
...
-(BOOL)validate{
    ...
    for (int i=0; i<[arrRegx count]; i++) {
        ...
        }else  if([dic objectForKey:@"distinct"]){
            NSString *txtConfirm=[dic objectForKey:@"distinct"];
            if([txtConfirm isEqualToString:self.text]){
                [self showErrorIconForMsg:[dic objectForKey:@"msg"]];
                return NO;
            }
...
-(void)showErrorIconForMsg:(NSString *)msg{
    ...
    [btnError setBackgroundImage:[[UIImage imageNamed:IconImageName] imageWithRenderingMode:(UIImageRenderingModeAlwaysTemplate)] forState:UIControlStateNormal];
    btnError.tintColor = [UIColor redColor];
